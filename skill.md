...
接收 CSV（兼容 Excel）表格数据，自动完成加载→画像→清洗→多维分析→洞察→输出。**专注企业经营数据（销售/财务/运营场景）**。无状态、无副作用。**LLM 只做解读，计算 100% 由 Pandas 完成**。

| ✅ 负责 | ❌ 不负责 |
|---------|-----------|
| 企业经营 CSV/Excel（销售/财务/运营）分析 | UI、文件上传 |
| 列语义识别（基于经营关键词字典） | 非结构化数据、PDF |
| 12 项多维分析(Pandas计算+LLM解读) | 数据库直连、多轮对话 |
| 洞察生成(3-8条) + 结构化输出 | 报告排版、PDF导出 |

> **适用数据**: 含收入/成本/利润/产品/区域/客户/渠道/日期等字段的经营表格。通用 CSV 列名不在字典中将被标记为 `ignore`，可通过 `column_hints` 手动标注扩展。
